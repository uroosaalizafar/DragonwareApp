...
class TechnologyState extends StatefulWidget {
  @override
  TechnologyStateful createState() => TechnologyStateful();
}

class TechnologyStateful extends State<TechnologyState> {
    ...
    return Expanded(
      child: Container(
        ...
        child: ListView.builder(
            ...
            itemCount: getTechnologyData.length ?? 0,
            itemBuilder: (_, index) {
              return InkWell(
                onTap: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => TechDetailScreen(
                            title: getTechnologyData[index].title,
                            subtitle: getTechnologyData[index].subtitle,
                            desc: getTechnologyData[index].desc,
                            price: getTechnologyData[index].price,
                            image: getTechnologyData[index].image,
                            review: getTechnologyData[index].review,
                          )));
                },
                child: Container(
                  width: mq.width * 0.6,
                    ...
                    child: ListTile(
                      title: Column(
                        children: [
                          ...
                          Row(
                            children: [
                              Text(getTechnologyData[index].price,style: TextStyle(color: appMainColor),),
                              Spacer(),
                              Text(getTechnologyData[index].review.toString(),style: TextStyle(color: appMainColor)),
                              Text("⭐"),
                            ],
                          ),
                        ],
